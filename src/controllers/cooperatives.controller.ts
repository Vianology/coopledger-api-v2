...
export async function createCooperative(req: Request, res: Response) {
  ...
  if (!name || !description || !founders || !statusDoc || !proofDoc || !identityDoc) return res.status(400).json({ message: "Missing required fields" });
  ...
  try {
    ...
    if (logo) {
      const uploadResult = await new Promise<any>((resolve, reject) => {
        cloudinary.uploader.upload_stream({ folder: "cooperatives", public_id: logo.originalname.split(".")[0] }, (error, result) => error ? reject(error) : resolve(result)).end(logo.buffer);
      });
      logoUrl = uploadResult.secure_url;
    }
    const result = await prisma.$transaction(async (tx) => {
      const created = await tx.cooperative.create({ data: { name, description, logo: logoUrl, founders: JSON.parse(founders), latitude: latitude ? parseFloat(latitude) : null, longitude: longitude ? parseFloat(longitude) : null, encryptionKey: encryptedCoopKey, creatorId: req.session.user.id, statusDocumentCid: statusCid, proofDocumentCid: proofCid, identityDocumentCid: identityCid, businessPlanDocumentCid: businessCid } });
      await tx.membership.create({ data: { userId: req.session.user.id, cooperativeId: created.id, status: MembershipStatus.ACCEPTED, grade: MembershipGrade.ADMIN, role: MembershipRole.FARMER } });
      return created;
    });
...
export async function getCooperatives(req: Request, res: Response) {
  ...
  if (lat && lng) {
    const userLat = parseFloat(lat as string), userLng = parseFloat(lng as string), rad = parseFloat(radius as string);
...
export async function joinCooperative(req: Request, res: Response) {
  ...
  try {
    const membership = await prisma.$transaction(async (tx) => {
      ...
      const existing = await tx.membership.findUnique({ where: { userId_cooperativeId: { userId: req.session.user.id, cooperativeId } } });
      if (existing) throw new HttpError(409, "Already exists or pending");
      return tx.membership.create({ data: { userId: req.session.user.id, cooperativeId, status: MembershipStatus.PENDING, grade: MembershipGrade.MEMBER, role: MembershipRole.FARMER } });
    });
    return res.status(201).json(membership);
  } catch (error) { if (error instanceof HttpError) return res.status(error.statusCode).json({ message: error.message }); return res.status(500).json({ message: "Internal error" }); }
}

export async function approveCooperativeJoin(req: Request, res: Response) {
  const { cooperativeId, memberId, isApproved } = req.body;
  if (!cooperativeId || !memberId || isApproved === undefined) return res.status(400).json({ message: "Missing fields" });
  try {
    await prisma.$transaction(async (tx) => {
      const requester = await tx.membership.findUnique({ where: { userId_cooperativeId: { userId: req.session.user.id, cooperativeId } } });
      if (!requester || requester.grade !== MembershipGrade.ADMIN) throw new HttpError(403, "Only admin can approve");
      const membership = await tx.membership.findUnique({ where: { userId_cooperativeId: { userId: memberId, cooperativeId } } });
      ...
      await tx.membership.update({ where: { userId_cooperativeId: { userId: memberId, cooperativeId } }, data: { status: isApproved ? MembershipStatus.ACCEPTED : MembershipStatus.REJECTED } });
    });
