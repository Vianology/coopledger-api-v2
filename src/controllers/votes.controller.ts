...
export async function proposeVote(req: Request, res: Response) {
  ...
  try {
    const vote = await prisma.$transaction(async (tx) => {
      ...
      if (!membership || (membership.grade !== MembershipGrade.ADMIN && membership.grade !== MembershipGrade.TREASURER)) {
        throw new HttpError(403, "Only bureau can propose");
      }
...
export async function castVote(req: Request, res: Response) {
  ...
  if (!voteId || choiceIndex === undefined) {
    return res.status(400).json({ message: "Missing voteId or choice" });
  }
  try {
    const voteCast = await prisma.$transaction(async (tx) => {
      ...
      if (!vote || vote.status !== VoteStatus.OPEN) {
        throw new HttpError(404, "Vote not open");
      }
      const membership = await tx.membership.findUnique({
        where: {
      ...
      if (!membership || membership.status !== MembershipStatus.ACCEPTED) {
        throw new HttpError(403, "Not active member");
      }
      const existing = await tx.voteCast.findUnique({
      ...
      await tx.membership.update({
        where: { userId_cooperativeId: { userId: req.session.user.id, cooperativeId: vote.cooperativeId } },
